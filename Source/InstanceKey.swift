...
internal final class InstanceKey<Service, Args> : Hashable  {
  ...
  var hashValue: Int {
    get {
      return ObjectIdentifier(serviceType).hashValue ^ ObjectIdentifier(argsType).hashValue
              ^ (key?.hashValue ?? 0)
    }
  }
  ...
  static func ==(lhs: InstanceKey<Service, Args>, rhs: InstanceKey<Service, Args>) -> Bool {
    ...
    let fieldsEqual = lhs.reuse == rhs.reuse && lhs.serviceType == rhs.serviceType && lhs.argsType == rhs.argsType
            && rhs.key == lhs.key
